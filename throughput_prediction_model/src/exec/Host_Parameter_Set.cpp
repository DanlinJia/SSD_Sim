...
unsigned int Host_Parameter_Set::Read_Weights = 1;
unsigned int Host_Parameter_Set::Write_Weights = 1;
...
void Host_Parameter_Set::XML_deserialize(rapidxml::xml_node<> *node)
{
	try {
		for (auto param = node->first_node(); param; param = param->next_sibling()) {
			...
			} else if (strcmp(param->name(), "Read_Weights")==0) {
				std::string val = param->value();
				Read_Weights = std::stoul(val);
			} else if (strcmp(param->name(), "Write_Weights")==0) {
				std::string val = param->value();
				Write_Weights = std::stoul(val);
			}
		}
	} catch (...) {
